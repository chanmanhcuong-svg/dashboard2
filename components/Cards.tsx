...
export function ZodiacCard({ sign }: { sign: ZodiacSign }) {
  ...
  return (
    <article className="glass rounded-3xl p-4 transition hover:-translate-y-1">
      ...
      <button type="button" className="mt-3 rounded-full border border-white/15 px-3 py-1 text-xs" onClick={() => { toggleFavorite(key); notify(saved ? t("unsave") : t("save")); }}>
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="flex justify-center gap-2">
        <button type="button" className="glass rounded-full px-3 py-1 text-xs" onClick={copyText}>{t("copy")}</button>
        <button type="button" className="glass rounded-full px-3 py-1 text-xs" onClick={download}>{t("download")}</button>
      </div>
    </div>
  );
}
